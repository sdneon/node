...
function patchProcessObject(expandArgv1) {
  ...
  if (expandArgv1 && process.argv[1]
    ...
    && !StringPrototypeStartsWith(process.argv[1], ':')) //SD: Direct script mode, not a path
  {
...
//SD BEGIN
function setupNodeJs4Cpp()
{
    global.process.versions.nodejs4Cpp = '1.0.0';
    ...
    //Add method to run any arbitrary script given, in global context
    global.runScript = function(script) {
        //codes modified from module.js to handle direct script string input instead of file path
        const CJSModule = require('internal/modules/cjs/loader').Module;
        const path = require('path');
        const cwd = require('internal/process/execution').tryGetCwd();

        //just a dummy filename
        const name = '.',
            filename = path.join(cwd, name);
        const module = new CJSModule(filename);
        module.filename = filename;
        module.paths = CJSModule._nodeModulePaths(cwd);

        const body = wrapForBreakOnFirstLine(script);
    ...
    global.process.NodeJs4Cpp = {
        //regular interval (in ms) of internal tick (to process C++ contributed scripts)
        tickIntervalMs: 100,
        /**
         * [Internal use] Regular tick to process C++ contributed scripts in queue.
         **/
        run: () => {
            let queue = global.process.getNodeJsTasks();
            if (!queue) { return; }
            let cnt = queue.length, i;
            let curTimeMs = {
                time: Date.now()
            };
            for (i = 0; i < cnt; ++i)
            {
                ...
                if (!task) { continue; }
                /**
                 * Inject the temporary input arguments array.
                 * It will only be valid for a short while,
                 * so script should quickly use/cache it.
                 * (As next task will replace it).
                 **/
                global.process.NodeJs4Cpp._inputs = task.inputs;
                if (typeof task.script === 'string')
                {
                    try
                    {
                        result = global.runScript(task.script); //eval
                    }
                    catch (ex)
                    {
                        console.log(`WRN: Failed to run c++ provided script owing to: ${ex.stack}`);
                    }
                    delete task.script; //cleanup
                }
                try
                {
                    if (task.callback)
                    {
                        global.process.callCppCallback(task.callback, task.extraInfo, result);
                    }
                    else if (task.callbackJsonObj)
                    {
                        global.process.callCppJsonObjCallback(task.callbackJsonObj, result);
                    }
                    else if (task.callbackJsonStr)
                    {
                        global.process.callCppJsonStrCallback(task.callbackJsonStr, JSON.stringify(result));
                    }
                }
                catch (ex)
                {
                    console.log(`WRN: Failed to return result to c++ callback owing to: ${ex.stack}`);
                }
            }
        },
        /**
         * [Internal use] remove regular tick.
         **/
        _disableTick: function()
        {
            removeInterval(global.process.NodeJs4Cpp.tickTimer);
            global.process.NodeJs4Cpp.tickTimer = false;
        },
        /**
        * Enable regular tick to process C++ contributed scripts in queue.
        **/
        enableTick: function(enable)
        {
            if (typeof global.process.getNodeJsTasks !== 'function')
            {
                return;
            }
            if ((enable === true) || (enable === undefined))
            {
                if (global.process.NodeJs4Cpp.tickTimer)
                {
                    return; //already ticking
                }
                global.process.NodeJs4Cpp.tickTimer = global.setInterval(() => {
                    global.process.NodeJs4Cpp.run();
                }, global.process.NodeJs4Cpp.tickIntervalMs);
            }
            else  //!enable
            {
                if (global.process.NodeJs4Cpp.tickTimer)
                {
                    global.process.NodeJs4Cpp._disableTick();
                }
            }
        },
        /**
         * [Internal use] Reschedule regular tick using current tickIntervalMs setting.
         **/
        _resetTick: function()
        {
            if (global.process.NodeJs4Cpp.tickTimer)
            ...
            global.process.NodeJs4Cpp.enableTick(true);
        },
        /**
         * Change regular tick interval (tickIntervalMs) and reschedule.
         **/
        setTickInterval: function(tickIntervalMs)
        {
            if (tickIntervalMs < 0)
            {
                tickIntervalMs = 0;
            }
            global.process.NodeJs4Cpp.tickIntervalMs = tickIntervalMs;
            global.process.NodeJs4Cpp._resetTick();
        },
        /**
         * Register a C++ callback/method in a given object (objName) or global.
         * Callback will receive a JSON string parameter.
         * @param cppCbRef (reference) C++ callback.
         * @param chName (string) Name of callback.
         * @param objName (optional string) Callback is registered in given object if provided;
         *                                  Or in global object o.w.
         *                                  objName can be like 'process.versions'.
         * @return (bool) whether callback is registered.
         **/
        registerJsonStrCallback: function (cppCbRef, cbName, objName) {
            let obj = global;
            ...
            obj[cbName] = function() //wrap cpp callback
            {
                try
                {
                    global.process.callCppJsonStrCallback(cppCbRef, JSON.stringify(Array.from(arguments)));
                }
                catch (ex)
                {
                    console.log(`ERR ${cbName}: ${ex.stack}`);
                }
            };
            return true;
        },
        /**
         * Register a C++ callback/method in a given object (objName).
         * Callback will receive a rapidjson object parameter.
         * @param cppCbRef (reference) C++ callback.
         * @param chName (string) Name of callback.
         * @param objName (optional string) Callback is registered in given object if provided;
         *                                  Or in global object o.w.
         ...
         * @return (bool) whether callback is registered.
         **/
        registerJsonObjCallback: function(cppCbRef, cbName, objName)
        {
            let obj = global;
            if (typeof objName === 'string') {
                try {
            ...
            obj[cbName] = function () //wrap cpp callback
            {
                try
                {
                    global.process.callCppJsonObjCallback(cppCbRef, arguments);
                }
                catch (ex)
                {
                    console.log(`ERR ${cbName}: ${ex.stack}`);
        ...
        /**
         * Register a C++ callback/method in a given object (objName).
         * Callback will receive a HANDLE_NODEJS_OBJ.
         * @param cppCbRef (reference) C++ callback.
         * @param cbExtraInfo (reference/long) Extra info for callback.
         * @param cbName (string) Name of callback.
         * @param objName (optional string) Callback is registered in given object if provided;
         *                                  Or in global object o.w.
         *                                  objName can be like 'process.versions'.
         * @return (bool) whether callback is registered.
         **/
        registerCallback: function (cppCbRef, cbExtraInfo, cbName, objName) {
            let obj = global;
            if (typeof objName === 'string') {
                try {
                    obj = eval(objName);
            ...
            {
                try {
                    global.process.callCppCallback(cppCbRef, cbExtraInfo,  Array.from(arguments));
                }
                catch (ex) {
                    console.log(`ERR ${cbName}: ${ex.stack}`);
                }
