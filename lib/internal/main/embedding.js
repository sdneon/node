...
const { StringPrototypeStartsWith } = primordials; //SD
...
function embedderRunCjs(content) {
  ...
  if (StringPrototypeStartsWith(content, '#ds')) { //SD
    let fname = filename || '';
    fname = fname.toLowerCase();
    content = transpiler.dscript.transpile(content, undefined, undefined, undefined, path.basename(filename));
  }
